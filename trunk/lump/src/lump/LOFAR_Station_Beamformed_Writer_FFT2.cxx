...
// code for the derived writer class for FFT spectra, case 2
//_HIST  DATE NAME PLACE INFO
// 2014 Aug 29  James M Anderson  --- start from LOFAR_Station_Beamformed_Writer_FFT0.cxx
// 2014 Sep 03  JMA  ---- rename from LOFAR_Station_Beamformed_Writer_FFT1.cxx to LOFAR_Station_Beamformed_Writer_FFT2.cxx
...
// Note that one extra "beamlet" is added to the processing to provide information
// on the spectral response in the event that not all samples are valid.  Thus,
// the last beamlet (beamlets_per_sample-1) does not represent real sky
// observations, but is instead is the Fourier transofrm of the validity time
// series of the samples, with valid samples having value 1.0 and invalid samples
// having value 0.0.  This time series is weighted and Fourier transformed in
// the same way as the  regular sky data are transformed.  The "0 frequency" value
// of the valid series transform is then used to scale (dividing the data values
// by the real value of the 0 frequency term of the valid series) the sky data and
// the valid series data transforms, before they are written out to disk.
//
//
// Information file:
...
//
// Valid file.  A single file is written out to indicate the weighted fraction of
...
// valid.  The validity of each sample is placed into an array, with 1 indicating
// valid data and 0 indicating invalid data.  This array is weighted the same
// as the regular station time voltage values, and then Fourier transformed, along
// with the regular station data arrays.  The real value at 0 frequency within the
// "valid array" Fourier transform is used for the output to the valid file.  The
// regular station data transforms and the valid value transforms are then
// divided by this value, to compensate for missing data (filled in with value 0
// by default).  
// The filename is
// filename_base.valid
//
...
LOFAR_Station_Beamformed_Writer_FFT2::
...
LOFAR_Station_Beamformed_Writer_Base(filename_base_,
    ...
    NUM_REALS_IN_FULL_STORAGE_BUFFER(uint_fast64_t(NUM_OUTPUT_CHANNELS_)*2*(NUM_Output_Beamlets_+1) * NUM_RSP_BEAMLET_POLARIZATIONS),
    NUM_Output_Beamlets_PV(NUM_Output_Beamlets_+1),
    NUM_PARALLEL_BEAMLETS((NUM_Output_Beamlets_+1) * NUM_RSP_BEAMLET_POLARIZATIONS),
    WRITE_VALID_INFO_BEAMLETS(false),
    ...
    window_TYPE(window_type_)
{
    ...
    if((write_valid_info_beamlets_)) {
        if( (write_valid_info_beamlets_[0] != '0')
           && (write_valid_info_beamlets_[0] != 'N')
           && (write_valid_info_beamlets_[0] != 'n')
           && (write_valid_info_beamlets_[0] != 'F')
           && (write_valid_info_beamlets_[0] != 'f')
            ) {
            WRITE_VALID_INFO_BEAMLETS = true;
        }
    }
    ...
    if(WRITE_VALID_INFO_BEAMLETS) {
        // This writer type adds an extra beamlet to the output to show what the
        // valid timeseries transform looks like
        append_valid_information_beamlet_info_to_info_files();
    }
...
int_fast32_t LOFAR_Station_Beamformed_Writer_FFT2::
...
set_up_work_buffer_areas() restrict throw()
{
    ...
    FFT_BUFFER_LENGTH_CHAR = uint_fast64_t(NUM_RSP_BEAMLET_POLARIZATIONS)
        * NUM_Output_Beamlets_PV * NUM_REALS_IN_FFT_BLOCK;
    ...
    FFT_BUFFER_LENGTH_CHAR_NOV = uint_fast64_t(NUM_RSP_BEAMLET_POLARIZATIONS)
        * NUM_Output_Beamlets * NUM_REALS_IN_FFT_BLOCK;
    OUTPUT_BUFFER_LENGTH_CHAR_NOV = FFT_BUFFER_LENGTH_CHAR_NOV;
    FFT_BUFFER_LENGTH_CHAR_NOV *= LOFAR_raw_data_type_enum_size(processing_DATA_TYPE_REAL);
    OUTPUT_BUFFER_LENGTH_CHAR_NOV *= LOFAR_raw_data_type_enum_size(output_DATA_TYPE_REAL);
    ...
    // If not writing the valid info beamlets, adjust the output size variable
    if(!WRITE_VALID_INFO_BEAMLETS) {
        OUTPUT_BUFFER_LENGTH_CHAR_NOV = uint_fast64_t(NUM_RSP_BEAMLET_POLARIZATIONS)
                                        * NUM_Output_Beamlets * NUM_REALS_IN_FFT_BLOCK;
        OUTPUT_BUFFER_LENGTH_CHAR_NOV *= LOFAR_raw_data_type_enum_size(output_DATA_TYPE_REAL);
    }
    return 0;
...
set_up_FFTW3() restrict throw()
{
    ...
    int parallel = int(NUM_PARALLEL_BEAMLETS);
...
write_header_init() restrict throw()
{
    ...
    Info.set_beamlets_per_sample(uint32_t(NUM_Output_Beamlets_PV));
