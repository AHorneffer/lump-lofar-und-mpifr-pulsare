...
// code for the polyphase filterbank spectra power, case 1
//_HIST  DATE NAME PLACE INFO
// 2014 Sep 03  James M Anderson  --GFZ  start from LOFAR_Station_Beamformed_Writer_P_PFB0.cxx
// 2014 Sep 04  JMA  ---- rename PFB_samples to PFB_blocks, round for num
...
// Power PFB Format 0       polyphase filterbank power output
...
LOFAR_Station_Beamformed_Writer_P_PFB1::
...
LOFAR_Station_Beamformed_Writer_Base(filename_base_,
    ...
    weighted_valid_PFB_blocks_in_this_integration_p(0),
...
int_fast32_t LOFAR_Station_Beamformed_Writer_P_PFB1::
...
check_input_parameters() restrict throw()
{
    ...
    // Check the number of samples per integration
    {
        ...
        if(num_FFT_blocks > Real64_t(0xFFFFFFFFU)) {
            DEF_LOGGING_CRITICAL("too many PFB blocks per integration to fit within 32 bit integer --- desired_seconds_per_integration was %E, LINES_PER_SECOND was %E, NUM_OUTPUT_CHANNELS was %d, result %E\n", desired_seconds_per_integration, LINES_PER_SECOND, int(NUM_OUTPUT_CHANNELS), num_FFT_blocks);
            exit(2);
        }
        PFB_BLOCKS_PER_INTEGRATION = uint_fast32_t(llrint(num_FFT_blocks));
        if(PFB_BLOCKS_PER_INTEGRATION == 0) {
            PFB_BLOCKS_PER_INTEGRATION++;
        }
    }
...
set_up_work_buffer_areas() restrict throw()
{
    ...
    // Second, the integration storage area
    ...
    uint_fast64_t PFB_TAP_BUFFER_LENGTH_CHAR = FFT_BUFFER_LENGTH_CHAR + NUM_REALS_IN_FFT_BLOCK *  LOFAR_raw_data_type_enum_size(processing_DATA_TYPE_REAL);
    ...
    // Fourth, the weighted valid PFB blocks counter
    size_t data_type_max_size = LOFAR_raw_data_type_enum_size(L_RAW_DATA_TYPE_MAX);
    retval = posix_memalign(&weighted_valid_PFB_blocks_in_this_integration_p, MPIFR_NUMERICAL_DEFAULT_DATA_ALIGNMENT, data_type_max_size);
    if((retval)) {
        DEF_LOGGING_ERROR("Could not allocate %llu bytes for weighted_valid_PFB_blocks_in_this_integration_p with retval %d\n", (unsigned long long)(data_type_max_size), retval);
        return -1;
    }
    memset(weighted_valid_PFB_blocks_in_this_integration_p, 0, data_type_max_size);
...
write_header_init() restrict throw()
{
    ...
    Real64_t seconds_per_integration(seconds_per_FFT*PFB_BLOCKS_PER_INTEGRATION);
    uint64_t RAW_SAMPLES_PER_FULL_INTEGRATION(uint64_t(NUM_OUTPUT_CHANNELS)*(uint64_t(PFB_BLOCKS_PER_INTEGRATION)+1));
    ...
    Info.set_samples_per_integration(uint32_t(RAW_SAMPLES_PER_FULL_INTEGRATION));
    ...
    Info.set_seconds_per_integration_full(RAW_SAMPLES_PER_FULL_INTEGRATION*SECONDS_PER_LINE);
    Info.set_seconds_per_integration_eff(seconds_per_integration);
    Info.set_samples_per_integration_eff(uint64_t(NUM_OUTPUT_CHANNELS)*PFB_BLOCKS_PER_INTEGRATION);
    Info.set_seconds_per_output(seconds_per_integration);
    if((scale_by_inverse_num_samples)) {
        Info.set_integration_multiplier(SCALING_MULTIPLIER/PFB_BLOCKS_PER_INTEGRATION);
    }
    else {
...
write_integration_flush() restrict throw()
{
    ...
    while(PFB_blocks_in_this_integration != 0) {
        valid_samples_in_this_PFB = 0;
        switch(processing_DATA_TYPE_REAL) {
        case L_Real16_t:
            DEF_LOGGING_CRITICAL("Real16_t processing for FFTs is not yet supported.  Contact your developer.\n");
