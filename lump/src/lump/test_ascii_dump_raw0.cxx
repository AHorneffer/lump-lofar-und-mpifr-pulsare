...
// test code to convert a beamlet to ASCII for raw0 format
...
// see LOFAR_Station_Beamformed_Writer_Raw0.cxx for format details
...
namespace MPIfR {
namespace LOGGING {
extern const char PROGRAM_NAME[] = "test_ascii_dump_raw0";
...
int main(const int argc, const char* const argv[])
{
    ...
    if(info.get_WRITER_TYPE() != uint16_t(MPIfR::LOFAR::Station::LOFAR_DOFF_RAW0_OUT)) {
        fprintf(stderr, "Error: wrong data type '%u' in %s:%d\n",
    ...
    if(info.get_output_DATA_TYPE() != int32_t(MPIfR::LOFAR::L_intComplex32_t)) {
    ...
    const size_t sample_line_size = sizeof(int16_t)*NUM_INT;
    int16_t line[NUM_INT];
    const size_t o = 4*beamlet;
    ...
    const uint64_t num_samples = info.get_num_output_timeslots();
