...
// for each time sample.  ALL BEAMLETS ARE PROCESSED.
//
//
// Information file:
...
LOFAR_Station_Beamformed_Writer_Raw::
...
int_fast32_t LOFAR_Station_Beamformed_Writer_Raw::
thread_constructor() restrict
{
    ...
    if(NUM_Output_Beamlets != NUM_Beamlets) {
        DEF_LOGGING_CRITICAL("hardcoded to process all beamlets, but told to process fewer in\n");
        exit(1);
    }
    // Now open the files
        ...
        for(uint_fast32_t f=0; f < NUM_FP_DATA; f++) {
            if((fp_data[f])) {
                ...
                if((retval)) {
                    ...
                    DEF_LOGGING_ERROR("Error: closing '%s' data %u failed\n", filename_base, (unsigned int)(f));
...
write_standard_packets() restrict throw()
{
    ...
    case 2:
        for(uint_fast32_t packet=0; packet < current_num_packets; packet++) {
            const uint16_t* restrict data_16_p_in =
                reinterpret_cast<const uint16_t* const restrict>(current_data_packets[packet]->data_start_const());
            for(uint_fast32_t beamlet=0,fp=0; beamlet < NUM_Beamlets; beamlet++) {
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0; block < NUM_Blocks; block++) {
                    data_16_p[block] = *data_16_p_in++;
                    data_16_p[NUM_Blocks+block] = *data_16_p_in++;
                }
                // X
    ...
    case 4:
        for(uint_fast32_t packet=0; packet < current_num_packets; packet++) {
            const uint32_t* restrict data_32_p_in =
                reinterpret_cast<const uint32_t* const restrict>(current_data_packets[packet]->data_start_const());
            for(uint_fast32_t beamlet=0,fp=0; beamlet < NUM_Beamlets; beamlet++) {
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0; block < NUM_Blocks; block++) {
                    data_32_p[block] = *data_32_p_in++;
                    data_32_p[NUM_Blocks+block] = *data_32_p_in++;
                }
                // X
    ...
    case 8:
        for(uint_fast32_t packet=0; packet < current_num_packets; packet++) {
            const uint64_t* restrict data_64_p_in =
                reinterpret_cast<const uint64_t* const restrict>(current_data_packets[packet]->data_start_const());
            for(uint_fast32_t beamlet=0,fp=0; beamlet < NUM_Beamlets; beamlet++) {
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0; block < NUM_Blocks; block++) {
                    data_64_p[block] = *data_64_p_in++;
                    data_64_p[NUM_Blocks+block] = *data_64_p_in++;
                }
                // X
    ...
    case 16:
        for(uint_fast32_t packet=0; packet < current_num_packets; packet++) {
            const intComplex128_t* restrict data_128_p_in =
                reinterpret_cast<const intComplex128_t* const restrict>(current_data_packets[packet]->data_start_const());
            for(uint_fast32_t beamlet=0,fp=0; beamlet < NUM_Beamlets; beamlet++) {
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0; block < NUM_Blocks; block++) {
                    data_128_p[block] = *data_128_p_in++;
                    data_128_p[NUM_Blocks+block] = *data_128_p_in++;
                }
                // X
    ...
    case 1:
        {
            ...
            for(uint_fast32_t beamlet=0,fp=0,offset=0; beamlet < NUM_Beamlets;
                beamlet++, offset+=NUM_COMPLEX_VOLTAGE_POINTS_PER_BEAMLET) {
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0,block_offset=0; block < NUM_ACTUAL_BLOCKS; block++) {
                    data_8_p[block] = data_8_p_in[offset+block_offset++];
                    data_8_p[NUM_Blocks+block] = data_8_p_in[offset+block_offset++];
                }
                // X
    ...
    case 2:
        {
            ...
            for(uint_fast32_t beamlet=0,fp=0,offset=0; beamlet < NUM_Beamlets;
                ...
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0,block_offset=0; block < NUM_ACTUAL_BLOCKS; block++) {
                    data_16_p[block] = data_16_p_in[offset+block_offset++];
                    data_16_p[NUM_Blocks+block] = data_16_p_in[offset+block_offset++];
                }
                // X
    ...
    case 4:
        {
            ...
            for(uint_fast32_t beamlet=0,fp=0,offset=0; beamlet < NUM_Beamlets;
                ...
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0,block_offset=0; block < NUM_ACTUAL_BLOCKS; block++) {
                    data_32_p[block] = data_32_p_in[offset+block_offset++];
                    data_32_p[NUM_Blocks+block] = data_32_p_in[offset+block_offset++];
                }
                // X
    ...
    case 8:
        {
            ...
            for(uint_fast32_t beamlet=0,fp=0,offset=0; beamlet < NUM_Beamlets;
                ...
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0,block_offset=0; block < NUM_ACTUAL_BLOCKS; block++) {
                    data_64_p[block] = data_64_p_in[offset+block_offset++];
                    data_64_p[NUM_Blocks+block] = data_64_p_in[offset+block_offset++];
                }
                // X
    ...
    case 16:
        {
            ...
            for(uint_fast32_t beamlet=0,fp=0,offset=0; beamlet < NUM_Beamlets;
                ...
                // copy over polarizations, X to low, Y to high
                for(uint_fast16_t block=0,block_offset=0; block < NUM_ACTUAL_BLOCKS; block++) {
                    data_128_p[block] = data_128_p_in[offset+block_offset++];
                    data_128_p[NUM_Blocks+block] = data_128_p_in[offset+block_offset++];
                }
                // X
