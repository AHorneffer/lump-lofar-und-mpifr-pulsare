...
// Derived class for writing polyphase filterbank spectra, case 1
//_HIST  DATE NAME PLACE INFO
// 2014 Sep 03  James M Anderson  --GFZ  start from LOFAR_Station_Beamformed_Writer_PFB0.h
...
#ifndef LOFAR_Station_Beamformed_Writer_PFB1_H
#define LOFAR_Station_Beamformed_Writer_PFB1_H
...
//_CLASS  LOFAR_Station_Beamformed_Writer_PFB1
class LOFAR_Station_Beamformed_Writer_PFB1 :
...
public:
    LOFAR_Station_Beamformed_Writer_PFB1(const char* const restrict filename_base_,
    ...
    virtual ~LOFAR_Station_Beamformed_Writer_PFB1();
...
protected:
    ...
    template<typename Tproc> int_fast32_t PFB_block() restrict throw()
    {
        ...
        return dump_PFB_to_output<Tproc>(Weighted_Valid_Sum);
    }
...
private:
    ...
    static uint16_t WRITER_TYPE() throw() {return uint16_t(LOFAR_DOFF_PFB1_OUT);}
    ...
    template<typename Tproc> int_fast32_t dump_PFB_to_output(const Tproc Weighted_Valid_Sum) restrict throw()
    {
        // First, if some of the data points were non-valid, then
        // scale the results up by the inverse of the Weighted_Valid_Sum,
        // before writing the data out.
        const bool NEED_TO_SCALE = ((valid_samples_in_this_PFB != NUM_SAMPLES_IN_FULL_PFB) && (valid_samples_in_this_PFB > 0));
        const Tproc valid_scale_factor = (NEED_TO_SCALE) ? 1.0/Weighted_Valid_Sum : 1.0;
            ...
            case L_Real16_t:
                {
                    ...
                    if(NEED_TO_SCALE) {
                        ...
                        for(uint_fast64_t i=0; i < NUM_REALS_IN_FULL_STORAGE_BUFFER; i++) {
                            data[i] = MPIfR::DATA_TYPE::Real16_t(fft_out[i]);
                        }
                    }
        ...
        if(valid_object->write_valid(Real32_t(Weighted_Valid_Sum)) == 0){
        }
        else {
            goto write_data_error;
        }
    ...
    // prevent copying
    LOFAR_Station_Beamformed_Writer_PFB1(const LOFAR_Station_Beamformed_Writer_PFB1& a);
    LOFAR_Station_Beamformed_Writer_PFB1& operator=(const LOFAR_Station_Beamformed_Writer_PFB1& a);
...
#endif // LOFAR_Station_Beamformed_Writer_PFB1_H
