// test_ascii_dump_P_PFB0.cxx
// test code to convert a beamlet to ASCII for P_PFB0 format
//_HIST  DATE NAME PLACE INFO
// 2014 Aug 29  James M Anderson  --- start from test_ascii_dump_PFB0.cxx
...
namespace MPIfR {
namespace LOGGING {
extern const char PROGRAM_NAME[] = "test_ascii_dump_P_PFB0";
...
template<typename T> int process(const int argc, const char* const argv[])
{
    ...
    if((info.get_WRITER_TYPE() != uint16_t(MPIfR::LOFAR::Station::LOFAR_DOFF_POWER_PFB0_OUT))
      && (info.get_WRITER_TYPE() != uint16_t(MPIfR::LOFAR::Station::LOFAR_DOFF_POWER_FFT0_OUT))
      && (info.get_WRITER_TYPE() != uint16_t(MPIfR::LOFAR::Station::LOFAR_DOFF_POWER_PFB1_OUT))
      && (info.get_WRITER_TYPE() != uint16_t(MPIfR::LOFAR::Station::LOFAR_DOFF_POWER_FFT1_OUT))
       )
    {
    ...
    const size_t NUM_R = 4*NUM_CHANNELS*NUM_BEAMLETS;
    const size_t sample_line_size = sizeof(T)*NUM_R;
    T line[NUM_R];
    const size_t o = 4*NUM_CHANNELS*o_index;
    ...
    for(uint64_t sample=0; sample < num_integrations; sample++) {
        ...
        for(size_t c=0; c < NUM_CHANNELS; c++) {
            printf("%.14E %.14E  %4d  %+14.7E %+14.7E     %+14.7E %+14.7E    %+14.7E\n",
                   ...
                   Real64_t(line[o+4*c]), Real64_t(line[o+4*c+1]), Real64_t(line[o+4*c+2]), Real64_t(line[o+4*c+3]),
                   v);
        }
        // size_t pos = 0;
