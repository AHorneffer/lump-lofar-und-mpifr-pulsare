...
// Derived class for writing polyphase filterbank spectra power, case 1
//_HIST  DATE NAME PLACE INFO
// 2014 Sep 03  James M Anderson  --GFZ  start from LOFAR_Station_Beamformed_Writer_P_PFB0.h
...
#ifndef LOFAR_Station_Beamformed_Writer_P_PFB1_H
#define LOFAR_Station_Beamformed_Writer_P_PFB1_H
...
//_CLASS  LOFAR_Station_Beamformed_Writer_P_PFB1
class LOFAR_Station_Beamformed_Writer_P_PFB1 :
...
public:
    LOFAR_Station_Beamformed_Writer_P_PFB1(const char* const restrict filename_base_,
    ...
    virtual ~LOFAR_Station_Beamformed_Writer_P_PFB1();
...
protected:
    ...
    void* weighted_valid_PFB_blocks_in_this_integration_p;
    ...
    template<typename Tproc> int_fast32_t PFB_block() restrict throw()
    {
        ...
        // accumulate the sum.  
        const Tproc ZERO(0.0);
        Tproc Weighted_Valid_Sum(ZERO);
        Tproc Weighted_Valid_Sum_Inner(ZERO);
        ...
        for(uint_fast16_t pb=0, pt=NUM_TAPS_m1; pb < NUM_TAPS_d2; pb++,pt--) {
            {
                ...
                // The valid values are stored in the fft_in_storage buffer
                // past the end of the actual beamlet data.  We arrived at
                // the end of the actual data above.  Correct for the window
                // mucking to get the zero frequency at the center.
                for(uint_fast32_t i=0; i < NUM_REALS_IN_FFT_BLOCK; i+=4) {
                    Weighted_Valid_Sum_Inner += data_in[i  ] * window[i  ];
                    Weighted_Valid_Sum_Inner -= data_in[i+2] * window[i+2];
                }
                Weighted_Valid_Sum += Weighted_Valid_Sum_Inner;
            }
            {
        ...
        return add_PFB_to_integration<Tproc>(Weighted_Valid_Sum);
    }
    ...
    template<class Tproc> int_fast32_t write_real_to_pfb_in_flush(uint_fast32_t* const restrict num_flushed) restrict throw()
    {
        ...
        *num_flushed = 0;
        ...
        for(uint_fast16_t p=0; p < NUM_TAPS_m1; p++) {
            ...
            // point formats
            memset(pfb_storage_tap[0],0,FFT_BUFFER_LENGTH_CHAR+uint_fast64_t(NUM_REALS_IN_FFT_BLOCK)*LOFAR_raw_data_type_enum_size(processing_DATA_TYPE_REAL));
    ...
    template<class Tproc> int_fast32_t add_PFB_to_integration(const Tproc Weighted_Valid_Sum) restrict throw()
    {
        ...
        const Tproc valid_scale_factor = (valid_samples_in_this_PFB != NUM_SAMPLES_IN_FULL_PFB) ? (Weighted_Valid_Sum*Weighted_Valid_Sum) : 1.0;
        ...
        // DEF_LOGGING_DEBUG("valid value = %+14.7E for writer %d, simple estimate was %+14.7E", Weighted_Valid_Sum, id, Real32_t((valid_samples_in_this_PFB==NUM_SAMPLES_IN_FULL_PFB) ? 1.0f : Real64_t(valid_samples_in_this_PFB)/Real64_t(NUM_SAMPLES_IN_FULL_PFB)));
        ...
        // Only add this PFB block in when the PFB block had
        // valid data.
        if(valid_samples_in_this_PFB > 0) {
            ...
            Tproc* iwp = static_cast<Tproc*>(weighted_valid_PFB_blocks_in_this_integration_p);
    ...
    {
        Tproc* iwp = static_cast<Tproc*>(weighted_valid_PFB_blocks_in_this_integration_p);
        Real32_t fraction_valid((valid_PFB_blocks_in_this_integration==PFB_BLOCKS_PER_INTEGRATION) ? 1.0f : (*iwp)/Real64_t(PFB_BLOCKS_PER_INTEGRATION));
        const Tproc SAMPLE_SCALING((*iwp > Tproc(0)) ?
                                   ( (scale_by_inverse_num_samples) ?
                                     1.0/(*iwp)
                                     : PFB_BLOCKS_PER_INTEGRATION/(*iwp) )
                                   : 0.0);
...
private:
    ...
    static uint16_t WRITER_TYPE() throw() {return uint16_t(LOFAR_DOFF_POWER_PFB1_OUT);}
    ...
    // prevent copying
    LOFAR_Station_Beamformed_Writer_P_PFB1(const LOFAR_Station_Beamformed_Writer_P_PFB1& a);
    LOFAR_Station_Beamformed_Writer_P_PFB1& operator=(const LOFAR_Station_Beamformed_Writer_P_PFB1& a);
...
#endif // LOFAR_Station_Beamformed_Writer_P_PFB1_H
